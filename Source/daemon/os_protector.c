...
int main(void)
{
	...
	sprintf(insmod_buffer, "/sbin/insmod /usr/local/lib/os_protector/shadow_box_helper.ko");
	...
	sprintf(insmod_buffer, "/sbin/insmod /usr/local/lib/os_protector/shadow_box.ko");
	error = system(insmod_buffer);
	if ((error == 127) || (error == -1))
	{
